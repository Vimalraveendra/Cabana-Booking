...
export const createApp = (options:AppOptions = {}) => {
  ...
  try {
    ...
    console.log(`Failed to read booking file: ${bookingsFile}`);
    process.exit(1);
  }
  ...
  const parseCabanas = (mapLines:string[]) => {
    const cabanas = [];
  ...
  const cabanas:Cabana[] = parseCabanas(mapLines);

  //---In-memory cabana booking state---
  //Map from cabanaId->booking info or null

  const cabanaState:CabanaState = {};
  ...
  app.get("/api/map", (_req:Request, res:Response<MapResponse>) => {
    const grid = mapLines.map((line) => line.split(""));
    const cabanaWithStatus:CabanaWithStatus[] = cabanas.map(({ id, row, col }) => ({
      id,
      row,
      col,
  ...
  app.post("/api/book", (req:Request<{},BookingResponse|ErrorResponse,BookingRequest> ,res:Response) => {
